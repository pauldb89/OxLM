...
TEST(QueryCacheTest, TestBasic) {
  ...
  EXPECT_EQ(make_pair(0.0, false), cache.get(query));

  cache.put(query, 0.5);
  EXPECT_EQ(1, cache.size());
  EXPECT_EQ(make_pair(0.5, true), cache.get(query));

  cache.put(query, 0.7);
  EXPECT_EQ(1, cache.size());
  EXPECT_EQ(make_pair(0.7, true), cache.get(query));
}
...
TEST(QueryCacheTest, TestSerialization) {
  ...
  EXPECT_EQ(make_pair(0.5, true), cache_copy.get(query));
}
