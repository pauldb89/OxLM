...
public:
    HiddenVariableLM();
    HiddenVariableLM(const ModelData& config, const Dict& hidden_labels,
                     const Dict& target_labels, const std::vector<int>& classes);
    ...
    void reinitialize(const ModelData& config, const Dict& hidden_labels,
                      const Dict& target_labels, const std::vector<int>& classes);
    void expandSource(const Dict& hidden_labels);

    int output_types() const {
        return m_target_labels.size();
    }
    int context_types() const {
        return m_target_labels.size();
    }
    int source_types() const {
        return m_hidden_labels.size();
    }

    int labels() const {
        return m_target_labels.size();
    }
    const Dict& label_set() const {
        return m_target_labels;
    }
    Dict& label_set() {
        return m_target_labels;
    }
    const Dict& source_label_set() const {
        return m_hidden_labels;
    }
    Dict& source_label_set() {
        return m_hidden_labels;
    }
    ...
    WordId label_id(const Word& l) const {
        return m_target_labels.Lookup(l);
    }

    const Word& label_str(WordId i) const {
        return m_target_labels.Convert(i);
    }
...
typedef std::shared_ptr<HiddenVariableLM> HiddenVariableLMPtr;
